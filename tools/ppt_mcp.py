"""從 ppt-workflow（獨立部署的可攜式 agent）經 MCP 載入簡報工具。

設計重點：
- erp-agent **不 import ppt 的任何實作**，只透過 MCP 契約（sse / stdio）連到一個跑起來的
  ppt-workflow 服務，取得 make_deck 工具。換版本只改 URL，不汙染。
...
環境變數（見 .env.example）：
    ...
    PPT_AGENT_URL         transport=sse 時連的位址（預設 http://ppt-workflow:8000/sse）
    PPT_AGENT_DIR/PYTHON  transport=stdio 時 ppt-workflow 專案路徑與其 venv python
    OUTPUTS_DIR           ppt-workflow 寫檔的目錄；需與 api 的 /files 指到同一處
"""
...
# ppt-workflow 寫檔目錄；stdio 模式由本模組 spawn 子行程，預設指到 erp 的 generated/（/files 服務的同一處）。
_DEFAULT_OUTPUTS = str(Path(__file__).resolve().parent.parent / "generated")
# ppt-workflow 是 sibling 的獨立服務（預設 ../agents/ppt-workflow，相對於 erp-agent）。
# 注意：這個路徑只給 stdio 退路（本機 spawn）與 start.sh 用；sse 模式只認 PPT_AGENT_URL，不碰路徑。
_ERP_ROOT = Path(__file__).resolve().parent.parent
_PPT_DIR = os.getenv("PPT_AGENT_DIR", str(_ERP_ROOT.parent / "agents" / "ppt-workflow"))
...
def _server_conf():
    ...
    if transport == "sse":
        return {"url": os.getenv("PPT_AGENT_URL", "http://ppt-workflow:8000/sse"),
                "transport": "sse"}
    ...
    return {"command": py, "args": ["mcp_server.py"],
...
def load_ppt_tools():
    """載入並快取 ppt-workflow 的 MCP 工具（已同步化）。失敗回空清單（不讓整個 app 起不來）。"""
    global _TOOLS
    ...
    try:
        ...
        print(f"[ppt_mcp] 載入 ppt-workflow MCP 工具失敗，ppt 暫時無工具可用：{type(e).__name__}: {e}")
        _TOOLS = []
    return _TOOLS
